...
```
攻击槽位可选择：
├─ 武者: powerStrike (1.9x, 消耗20)
├─ 炼气: qiBlade (3.0x, 消耗28)
├─ 筑基: foundationSword (4.5x, 消耗40)
├─ 金丹: goldenCoreBurst (6.5x, 消耗52)
├─ 元婴: nascentSoulStrike (9.0x, 消耗65)
└─ 化神: deitySlash (13.0x, 消耗80) ⭐ 推荐

防御槽位可选择：
├─ 武者: ironSkin (45%减伤, 消耗25)
├─ 炼气: qiShield (50%减伤, 消耗35)
├─ 筑基: foundationBarrier (60%减伤, 消耗45)
└─ ... (以此类推)
```
...
```
化神期玩家面对弱敌:
→ 装备武者的 powerStrike (消耗20, 1.9x)
→ 节省灵力，快速清理

化神期玩家面对BOSS:
→ 装备化神的 deitySlash (消耗80, 13.0x)
→ 全力输出，快速击杀
```
...
│  技能装备                            │
├─────────────────────────────────────┤
│  【攻击槽位】当前：powerStrike Lv.4  │
│   ○ powerStrike (武者) - 1.9x, 20   │
│   ○ qiBlade (炼气) - 3.0x, 28       │
│   ○ foundationSword (筑基) - 4.5x, 40│
│   ...                                │
│                                      │
│  【防御槽位】当前：ironSkin Lv.1     │
│   ○ ironSkin (武者) - 45%, 25       │
│   ○ qiShield (炼气) - 50%, 35       │
│   ...                                │
│                                      │
...
**过滤规则**：
- 只显示已学习的技能（level > 0）
- 只显示符合境界要求的技能（realmRequired <= currentRealm）
- 按类型分组显示
- 按境界从低到高排序
